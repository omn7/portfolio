...
export default function PublicNote({ params }: { params?: { userId: string; noteId: string } }) {
  const [dark, setDark] = useDark();
  const routeParams = useParams<{ userId: string; noteId: string }>();
  const userId = (params?.userId ?? routeParams?.userId) as string;
  const noteId = (params?.noteId ?? routeParams?.noteId) as string;
  ...
  if (error || !note) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center text-center px-6 font-sans" style={{ background: "var(--bg)", color: "var(--text)" }}>
        ...
        <Link href="/" className="inline-flex items-center justify-center bg-[var(--text)] text-[var(--bg)] rounded-full px-6 py-3 font-medium text-sm no-underline hover:opacity-90 transition-opacity">
          Return to home
        </Link>
      </main>
    );
  ...
  return (
    <main className="min-h-screen flex flex-col transition-colors duration-300" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      <header className="w-full max-w-[1000px] mx-auto px-6 py-6 sm:py-8 font-mono flex items-center justify-between">
        <Link href="/" className="text-[var(--text-alt)] hover:text-[var(--text)] hover:underline inline-block no-underline">
          ← back to index
        </Link>
