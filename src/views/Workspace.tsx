...
import {
  ...
  Bold, Italic, Underline, Link2, Type, Undo, Redo, BookOpen, Target, Palette
} from "lucide-react";
...
interface Todo {
  ...
  text: string;
  completed: boolean;
  category: "daily" | "weekly" | "monthly" | "goals";
...
type SidebarView = "dashboard" | "daily" | "weekly" | "monthly" | "finance" | "birthdays" | "notes" | "vocabulary" | "goals" | "whiteboard";
...
export default function Workspace() {
  ...
  const addTodo = async () => {
    ...
    const todo: Todo & { user_id: string } = {
      ...
      text: newTodo.trim(),
      completed: false,
      category: (sidebarView === "daily" || sidebarView === "weekly" || sidebarView === "monthly" || sidebarView === "goals") ? sidebarView : "daily",
  ...
  const getFilteredTodos = () => {
    ...
    if (sidebarView === "goals") {
      return todos.filter((t) => t.category === "goals");
    }
  ...
  const sidebarItems: { key: SidebarView; label: string; icon: any }[] = [
    { key: "dashboard", label: "Dashboard", icon: LayoutDashboard },
    { key: "goals", label: "Goals", icon: Target },
    { key: "whiteboard", label: "Project Planner", icon: Palette },
  ...
  return (
    <div className="min-h-screen font-mono flex" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      >
        ...
        <div className={`border-b border-[var(--text)] border-opacity-10 ${sidebarCollapsed ? "p-2" : "p-4"}`}>
          <div className="flex items-center justify-between">
            {!sidebarCollapsed && (
              <Link href="/" className="text-xs text-[var(--text-alt)] hover:text-[var(--text)] no-underline flex items-center gap-1">
          ...
          {!sidebarCollapsed && (
            <>
              ...
              <p className="text-xs text-[var(--text-alt)] truncate mt-0.5">{user?.email || "Guest Mode"}</p>
            </>
          )}
        </div>
        ...
        <div className={`border-t border-[var(--text)] border-opacity-10 ${sidebarCollapsed ? "p-1.5" : "p-3"}`}>
          <button
            ...
            title={sidebarCollapsed ? (user ? "Sign Out" : "Guest Mode") : undefined}
            disabled={!user}
          >
            <LogOut size={14} />
            {!sidebarCollapsed && (user ? "Sign Out" : "Guest Mode")}
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 min-h-screen min-w-0 flex flex-col">
        ...
        <header className="sticky top-0 z-30 border-b border-[var(--text)] border-opacity-10 px-4 sm:px-6 py-3 flex items-center justify-between" style={{ background: "var(--bg)" }}>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg sm:text-xl font-bold m-0">
              {sidebarView === "dashboard" ? "Dashboard" : sidebarView === "finance" ? "Finance Tracker" : sidebarView === "birthdays" ? "Birthdays" : sidebarView === "notes" ? "Notes" : sidebarView === "whiteboard" ? "Project Planner" : `${sidebarView.charAt(0).toUpperCase() + sidebarView.slice(1)} Tasks`}
            </h1>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className={`flex-1 ${sidebarView === "notes" || sidebarView === "dashboard" || sidebarView === "whiteboard" ? "" : "max-w-4xl mx-auto px-4 sm:px-6 py-6"}`}>
          ...
          {/* ──────────────── TASK VIEWS ──────────────── */}
          {(sidebarView === "daily" || sidebarView === "weekly" || sidebarView === "monthly" || sidebarView === "goals") && (
            <>
              {/* Add Task */}
          ...
          {sidebarView === "whiteboard" && (
            <div style={{ height: "100%", width: "100%", display: "flex", flex: 1 }}>
              <Excalidraw theme={dark ? "dark" : "light"} />
            </div>
          )}
                          ...
                          <div className="flex items-center border border-[var(--text)] border-opacity-10 rounded-lg sm:rounded-full overflow-hidden bg-[var(--bg)]">
                            <button
                              onClick={async () => {
                                ...
                                if (newPublic) {
                                  const url = `${window.location.origin}/notes/${user?.id || "guest"}/${activeNote.id}`;
